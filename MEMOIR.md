...
### 3-5. 분석 레이어 — 채용 시장 인사이트

단순 공고 저장에서 나아가 채용 시장 트렌드를 파악할 수 있는 경량 분석 레이어를 추가했습니다.

| 구성 요소 | 역할 |
|-----------|------|
| `db/analytics.py` | 유효 공고 기준 인기 태그·연봉·지역·경력 분포 집계 쿼리 |
| `analytics/snapshot.py` | 집계 결과를 `job_market_daily`에 날짜별 UPSERT |
| `main.py` 자동 연동 | 크롤링 완료 후 `save_snapshot()` 자동 실행 |
| `!인사이트` Discord 명령어 | 인기 스택 TOP 10 + 평균 연봉 + 지역·경력 분포 즉시 조회 |

`job_market_daily` 테이블에 `top_tags`, `region_dist`, `experience_dist`를 JSONB로 저장하여
과거 시점 재현이 가능하고, 추후 시계열 분석으로 확장 가능한 구조입니다.

Airflow·dbt 없이 cron + Python 스크립트로 ELT를 유지하여 운영 복잡도를 최소화했습니다.
...
주요 이슈 28건은 [TROUBLE_SHOOT.md](./TROUBLE_SHOOT.md)에 상세히 기록했습니다.
...
### 5-5. Discord 이벤트 루프 블로킹 — run_in_executor 적용

LLM 기능 추가 후 `!인사이트` 명령 입력 시 봇이 응답하지 않는 문제가 발생했습니다.
`expand_keyword()`, `rerank()` 등 LLM 호출 함수가 `requests.post()`를 사용하는 동기 함수인데,
이를 `async def`에서 직접 호출하면 Discord 이벤트 루프 전체가 블로킹됩니다.

`loop.run_in_executor(None, fn, *args)` 패턴으로 동기 함수를 스레드 풀에서 실행하여 해결했습니다.
이후 `bot.py`의 `sql_search()` 호출과 `notifier.py`의 `expand_keyword()` · `rerank()` 모두 동일 패턴으로 통일했습니다.
...
| 채용 시장 트렌드 파악 불가 | job_market_daily + analytics.py + !인사이트 | ✅ 완료 |
...
| 분석 레이어 | 집계 쿼리 + JSONB 스냅샷 (`job_market_daily`) |
